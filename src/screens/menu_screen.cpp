...
static const char* ITEM_LABELS[NUM_MENU_ITEMS] = { "Играть", "Настройки", "Выход" };
...
AppState updateMenu(MenuState& s, const Input& input) {
    ...
    if (input.confirmPressed || mouseSelect) {
        if (s.selectedItem == 0) return AppState::GAME;
        if (s.selectedItem == 1) return AppState::SETTINGS;
        if (s.selectedItem == 2) return AppState::EXIT;
    }
...
void drawMenu(Renderer& r, const MenuState& s) {
    ...
    for (int i = 0; i < NUM_MENU_ITEMS; i++) {
        ...
        if (sel)
            drawRect(r,
                MENU_ITEM_X - MENU_ITEM_W / 2.f, cy - MENU_ITEM_H / 2.f,
