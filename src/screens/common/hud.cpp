...
static const sf::Color ART_COLORS[5] = {
    {240, 200, 40, 255}, {40, 200, 240, 255}, {180, 60, 240, 255},
    {240, 120, 40, 255}, {60, 220, 80, 255},
};
...
void drawHUD(Renderer &r, const Player &p, const bool *artifacts,
  ...
  for (int i = 0; i < p.maxHp; i++) {
    ...
    if (r.hudHeartsLoaded) {
      if (full) {
        if (!blink)
          drawUISprite(r, r.hudHeartFullTex, hx, HP_HEARTS_Y);
      } else {
        drawUISprite(r, r.hudHeartEmptyTex, hx, HP_HEARTS_Y);
      }
    } else {
      sf::Color fill = full ? (blink ? sf::Color{220, 50, 50, 120}
                                     : sf::Color{220, 50, 50, 255})
  ...
  drawTextLeft(r, std::to_string(p.orbs), HP_HEARTS_X + 16.f, ORB_HUD_Y + 7.f,
               14, {255, 220, 80, 255});
  ...
  for (int i = 0; i < artifactCount; i++) {
    ...
    if (artifacts[i]) {
      ...
      drawRect(r, x + 1.f, ART_Y + 1.f, ART_SZ - 2.f, ART_SZ - 2.f,
               ART_COLORS[i]);
    } else {
