...
static bool hitTest(float cx, float cy, float hw, float hh, float mx, float my) {
    return mx >= cx - hw && mx <= cx + hw && my >= cy - hh && my <= cy + hh;
}

AppState updatePause(PauseState& s, const Input& input) {
    if (s.showConfirm) {
        bool hoverNo  = hitTest(CONFIRM_NO_CX,  CONFIRM_BTN_Y, CONFIRM_BTN_HW, CONFIRM_BTN_HH, input.mouseX, input.mouseY);
        bool hoverYes = hitTest(CONFIRM_YES_CX, CONFIRM_BTN_Y, CONFIRM_BTN_HW, CONFIRM_BTN_HH, input.mouseX, input.mouseY);

        s.isHovering = hoverNo || hoverYes;
        if (input.mouseMoved) {
            if (hoverNo)  s.confirmSel = 0;
            if (hoverYes) s.confirmSel = 1;
        }
        if (input.mouseClicked) {
            if (hoverNo)  { s.showConfirm = false; return AppState::PAUSE; }
            if (hoverYes) return AppState::EXIT;
        }

        if (input.leftPressed || input.rightPressed) s.confirmSel = 1 - s.confirmSel;
        if (input.escapePressed) { s.showConfirm = false; }
        else if (input.confirmPressed) {
            if (s.confirmSel == 1) return AppState::EXIT;
            s.showConfirm = false;
        }
    ...
    for (int i = 0; i < NUM_PAUSE_ITEMS; i++) {
        ...
        if (input.mouseX >= x0 && input.mouseX <= x0 + PAUSE_ITEM_W &&
            ...
            s.isHovering = true;
            if (!s.keyboardMode)    s.selectedItem = i;
            if (input.mouseClicked) { s.selectedItem = i; mouseSelect = true; }
        }
    }

    if (input.confirmPressed || mouseSelect) {
        switch (s.selectedItem) {
            case 0: return AppState::GAME;
            case 1: return AppState::MENU;
            case 2: return AppState::SETTINGS;
            case 3: s.showConfirm = true; s.confirmSel = 0; break;
        }
    }
...
void drawPause(Renderer& r, const PauseState& s) {
    ...
    for (int i = 0; i < NUM_PAUSE_ITEMS; i++) {
        ...
        if (sel)
            drawRect(r,
                PAUSE_ITEM_X - PAUSE_ITEM_W / 2.f, cy - PAUSE_ITEM_H / 2.f,
                PAUSE_ITEM_W, PAUSE_ITEM_H,
                {255, 255, 255, 25});
    ...
    if (s.showConfirm) {
        ...
        drawText(r, "Выйти из игры?", WINDOW_W / 2.f, CONFIRM_BY + 16.f, 16, {220, 220, 220, 255});

        sf::Color noCol  = (s.confirmSel == 0) ? sf::Color{255, 220, 80,  255} : sf::Color{140, 140, 160, 255};
        sf::Color yesCol = (s.confirmSel == 1) ? sf::Color{255, 100, 80,  255} : sf::Color{140, 140, 160, 255};
