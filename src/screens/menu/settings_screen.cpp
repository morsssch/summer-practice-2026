...
AppState updateSettings(SettingsState &s, const Input &input,
  ...
  if (s.selectedItem == 0) {
    if (input.leftPressed)
      data.volume = std::max(0, data.volume - 10);
    if (input.rightPressed)
      data.volume = std::min(100, data.volume + 10);
  }
...
void drawSettings(Renderer &r, const SettingsState &s,
  ...
  if (volSel)
    drawRect(r, SETTINGS_ITEM_X - SETTINGS_ITEM_W / 2.f,
             SETTINGS_ITEM_Y0 - SETTINGS_ITEM_H / 2.f, SETTINGS_ITEM_W,
             SETTINGS_ITEM_H, {255, 255, 255, 25});
  sf::Color volCol =
      volSel ? sf::Color{255, 220, 80, 255} : sf::Color{160, 160, 180, 255};
  ...
  if (backSel)
    drawRect(r, SETTINGS_ITEM_X - SETTINGS_ITEM_W / 2.f,
             backY - SETTINGS_ITEM_H / 2.f, SETTINGS_ITEM_W, SETTINGS_ITEM_H,
             {255, 255, 255, 25});
  sf::Color backCol =
      backSel ? sf::Color{255, 220, 80, 255} : sf::Color{160, 160, 180, 255};
