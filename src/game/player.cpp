                           ...
                           const Room& room, bool wasOnGround)
{
    ...
    if (p.vel.y > 0 && (isSolid(room, p.pos.x+4, p.pos.y+14) || isSolid(room, p.pos.x+12, p.pos.y+14))) {
        p.pos.y    = (float)((int)((p.pos.y+14) / 16) * 16) - 14;
        p.vel.y    = 0.f;
        p.onGround = true;
    } else if (p.vel.y > 0
        && (isPlatform(room, p.pos.x+4, p.pos.y+14) || isPlatform(room, p.pos.x+12, p.pos.y+14))
