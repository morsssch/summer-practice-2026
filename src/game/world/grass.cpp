...
void updateGrass(GrassField &g, const Player &player, const EnemyField &ef,
                 float dt) {
  g.time += dt;
  ...
  for (int i = 0; i < g.count; i++) {
    ...
    if (cx > s.rootX - 10.f && cx < s.rootX + 10.f && cy > s.rootY - 16.f &&
        cy < s.rootY + 4.f)
      s.angleVel += player.vel.x * 0.25f;

    for (int j = 0; j < ef.count; j++) {
      if (!ef.enemies[j].active)
        continue;
      float ex = ef.enemies[j].pos.x + 8.f;
      float ey = ef.enemies[j].pos.y + 8.f;
      if (ex > s.rootX - 10.f && ex < s.rootX + 10.f && ey > s.rootY - 16.f &&
          ey < s.rootY + 4.f)
