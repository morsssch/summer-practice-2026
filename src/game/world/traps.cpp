...
static bool allPlatesPressed(const TrapSystem &t, int group) {
  ...
  for (int i = 0; i < t.plateCount; i++) {
    ...
    if (!pl.active || pl.group != group)
      continue;
    anyPlate = true;
    if (!pl.pressed && !pl.locked)
      return false;
  }
  return anyPlate;
}
...
void updateTraps(TrapSystem &t, Room &room, Player &p, const PhysicsField &phys,
  ...
  for (int i = 0; i < t.plateCount; i++) {
    ...
    if (!pl.active || pl.locked)
      continue;
    float plx0 = pl.tx * 16.f, plx1 = plx0 + 16.f;
    float ply0 = pl.ty * 16.f, ply1 = ply0 + 16.f;
    bool byPlayer = (px1 > plx0 && px0 < plx1 && py1 > ply0 && py0 < ply1);
    bool byTile = false;
    for (int j = 0; j < phys.count && !byTile; j++) {
      if (!phys.tiles[j].active)
        continue;
      float sx0 = phys.tiles[j].pos.x + 1.f, sx1 = phys.tiles[j].pos.x + 15.f;
      float sy0 = phys.tiles[j].pos.y, sy1 = phys.tiles[j].pos.y + 16.f;
      byTile = (sx1 > plx0 && sx0 < plx1 && sy1 > ply0 && sy0 < ply1);
    }
    pl.pressed = byPlayer || byTile;
  }
