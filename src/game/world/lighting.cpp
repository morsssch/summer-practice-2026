...
static float rayBox(float ox, float oy, float dx, float dy, float bx, float by,
                    float bw, float bh) {
  const float EPS = 1e-6f;
  if (std::abs(dx) < EPS && (ox < bx || ox > bx + bw))
    return -1.f;
  if (std::abs(dy) < EPS && (oy < by || oy > by + bh))
    return -1.f;
  ...
  if (tmax < 0.f || tmin > tmax || tmin <= 0.f)
    return -1.f;
  return tmin;
}

float castRay(const Room &room, float ox, float oy, float dx, float dy,
              float maxDist, const LightOccluder *occ, int occCount) {
  float best = maxDist;

  const float EXPAND = 4.f;
  for (int i = 0; i < occCount; i++) {
    if (ox >= occ[i].x - EXPAND && ox <= occ[i].x + occ[i].w + EXPAND &&
        oy >= occ[i].y - EXPAND && oy <= occ[i].y + occ[i].h + EXPAND)
      continue;
...
void buildLightPolygon(const Room &room, float wx, float wy, float radius,
  ...
  for (int i = 0; i < occCount && nAngles + 12 <= MAX_ANGLES; i++) {
    if (wx >= occ[i].x - EXPAND && wx <= occ[i].x + occ[i].w + EXPAND &&
        wy >= occ[i].y - EXPAND && wy <= occ[i].y + occ[i].h + EXPAND)
      continue;

    float cx[4] = {occ[i].x, occ[i].x + occ[i].w, occ[i].x,
                   occ[i].x + occ[i].w};
    float cy[4] = {occ[i].y, occ[i].y, occ[i].y + occ[i].h,
                   occ[i].y + occ[i].h};
    for (int c = 0; c < 4; c++) {
