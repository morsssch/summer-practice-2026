...
static void updatePhysics(Player &p, const Input &input, float dt,
                          const Room &room, const PhysicsField &phys,
                          bool wasOnGround) {
  if (p.isDead) {
  ...
  if (p.iframeTimer > 0.f)
    p.iframeTimer -= dt;
  if (p.rollGraceTimer > 0.f)
    p.rollGraceTimer -= dt;
  if (p.facingCoyoteTimer > 0.f)
    p.facingCoyoteTimer -= dt;
  ...
  if (p.jumpBuffer > 0.f)
    p.jumpBuffer -= dt;
  if (p.coyoteTimer > 0.f)
    p.coyoteTimer -= dt;
  if (p.rollCooldown > 0.f)
    p.rollCooldown -= dt;
  if (p.attackTimer > 0.f)
    p.attackTimer -= dt;
  if (p.attackCooldown > 0.f)
    p.attackCooldown -= dt;
  ...
  if (rolling) {
    ...
    if (input.attackPressed && p.attackTimer <= 0.f &&
        p.attackCooldown <= 0.f) {
      p.attackTimer = ATTACK_DUR;
      p.attackCooldown = ATTACK_CD;
      bool atkDir =
          (p.facingCoyoteTimer > 0.f) ? p.lastFacingRight : p.facingRight;
    ...
    if (!input.left && !input.right) {
      if (p.vel.x > 0)
        p.vel.x = std::max(0.f, p.vel.x - FRIC * dt);
      if (p.vel.x < 0)
        p.vel.x = std::min(0.f, p.vel.x + FRIC * dt);
    }
  ...
  if (p.vel.x < 0 && (isSolid(room, p.pos.x + PLAYER_BOX_X, p.pos.y + 2) ||
                      isSolid(room, p.pos.x + PLAYER_BOX_X, p.pos.y + 12))) {
    p.pos.x =
        (float)((int)((p.pos.x + PLAYER_BOX_X) / 16) + 1) * 16 - PLAYER_BOX_X;
    p.vel.x = 0.f;
  }

  if (p.vel.x > 0 && (isSolid(room, p.pos.x + PLAYER_BOX_R, p.pos.y + 2) ||
                      isSolid(room, p.pos.x + PLAYER_BOX_R, p.pos.y + 12))) {
    p.pos.x = (float)((int)((p.pos.x + PLAYER_BOX_R) / 16)) * 16 - PLAYER_BOX_R;
    p.vel.x = 0.f;
  }
  ...
  if (p.vel.y < 0 && (isSolid(room, p.pos.x + 4, p.pos.y) ||
                      isSolid(room, p.pos.x + 12, p.pos.y))) {
    p.pos.y = (float)((int)(p.pos.y / 16) + 1) * 16;
    p.vel.y = 0.f;
  }
  if (p.vel.y > 0 && (isSolid(room, p.pos.x + 4, p.pos.y + 16) ||
                      isSolid(room, p.pos.x + 12, p.pos.y + 16) ||
                      physicsSolidAt(phys, p.pos.x + 4, p.pos.y + 16) ||
                      physicsSolidAt(phys, p.pos.x + 12, p.pos.y + 16))) {
    p.pos.y = (float)((int)((p.pos.y + 16) / 16) * 16) - 16;
    p.vel.y = 0.f;
    p.onGround = true;
  } else if (p.vel.y > 0 &&
             (isPlatform(room, p.pos.x + 4, p.pos.y + 16) ||
              isPlatform(room, p.pos.x + 12, p.pos.y + 16)) &&
...
void updatePlayer(Player &p, const Input &input, const Room &room,
  ...
  updatePhysics(p, input, dt, room, phys, wasOnGround);
  ...
  if (p.jumpFxTimer >= 0.f) {
    p.jumpFxTimer += dt;
    if (p.jumpFxTimer >= 4 * 0.07f)
      p.jumpFxTimer = -1.f;
  }
  if (p.hitAnimTimer >= 0.f) {
    p.hitAnimTimer += dt;
    if (p.hitAnimTimer >= 0.4f)
      p.hitAnimTimer = -1.f;
  }
  if (p.lostHeartTimer >= 0.f) {
    ...
    if (p.lostHeartFrameTimer >= 0.08f) {
      ...
      p.lostHeartFrame++;
      if (p.lostHeartFrame >= 5)
        p.lostHeartTimer = -1.f;
    }
  }
  if (p.gainedHeartTimer >= 0.f) {
    ...
    if (p.gainedHeartFrameTimer >= 0.08f) {
      ...
      p.gainedHeartFrame--;
      if (p.gainedHeartFrame < 0)
        p.gainedHeartTimer = -1.f;
    }
  }
...
void damagePlayer(Player &p, int amount, float knockbackX) {
  if (p.iframeTimer > 0.f || p.isDead || p.pendingDeath)
    return;
